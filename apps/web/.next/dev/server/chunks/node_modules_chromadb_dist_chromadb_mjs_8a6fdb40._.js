...
var Collection = class {
   ...
   */ async add(params) {
        ...
        await this.client.api.add(this.client.tenant, this.client.database, this.id, // TODO: For some reason the auto generated code requires metadata to be defined here.
        await prepareRecordRequest(params, this.embeddingFunction), this.client.api.options);
    }
    /**
   ...
   */ async upsert(params) {
        await this.client.init();
        await this.client.api.upsert(this.client.tenant, this.client.database, this.id, // TODO: For some reason the auto generated code requires metadata to be defined here.
        await prepareRecordRequest(params, this.embeddingFunction), this.client.api.options);
    }
    /**
   ...
   */ async count() {
        await this.client.init();
        return await this.client.api.count(this.client.tenant, this.client.database, this.id, this.client.api.options);
    }
    /**
   ...
   */ async get({ ids, where, limit, offset, include, whereDocument } = {}) {
        ...
        const resp = await this.client.api.aGet(this.id, this.client.tenant, this.client.database, {
            ids: idsArray,
            where,
            limit,
            offset,
   ...
   */ async update(params) {
        await this.client.init();
        await this.client.api.update(this.client.tenant, this.client.database, this.id, await prepareRecordRequest(params, this.embeddingFunction, true), this.client.api.options);
    }
    /**
   ...
   * ```
   *
   */ async query({ nResults = 10, where, whereDocument, include, queryTexts, queryEmbeddings }) {
        if (queryTexts && queryEmbeddings || !queryTexts && !queryEmbeddings) {
            throw new Error("You must supply exactly one of queryTexts or queryEmbeddings.");
        }
        await this.client.init();
        ...
        return await this.client.api.getNearestNeighbors(this.client.tenant, this.client.database, this.id, {
   ...
   */ async modify({ name, metadata }) {
        await this.client.init();
        return this.client.api.updateCollection(this.client.tenant, this.client.database, this.id, {
            new_name: name,
            new_metadata: metadata
        }, this.client.api.options).then(()=>{
            if (name !== void 0) {
                this.name = name;
            }
            if (metadata !== void 0) {
   ...
   */ async peek({ limit = 10 } = {}) {
        await this.client.init();
        return await this.client.api.aGet(this.id, this.client.tenant, this.client.database, {
            limit
        }, this.client.api.options);
    }
    /**
   ...
   */ async delete({ ids, where, whereDocument } = {}) {
        await this.client.init();
        let idsArray = void 0;
        if (ids !== void 0) idsArray = toArray(ids);
        await this.client.api.aDelete(this.id, this.client.tenant, this.client.database, {
            ids: idsArray,
            where,
            where_document: whereDocument
        }, this.client.api.options);
...
async function validateTenantDatabase(adminClient, tenant, database) {
    try {
        ...
        throw new Error(`Could not connect to tenant ${tenant}. Are you sure it exists? Underlying error:
${error}`);
    }
    try {
        ...
        throw new Error(`Could not connect to database ${database} for tenant ${tenant}. Are you sure it exists? Underlying error:
${error}`);
    }
}
function isBrowser() {
...
async function prepareRecordRequest(reqParams, embeddingFunction, update) {
    ...
    for(let i = 0; i < ids.length; i += 1){
        if (typeof ids[i] !== "string") {
            throw new Error(`Expected ids to be strings, found ${typeof ids[i]} at index ${i}`);
        }
    }
    ...
    if (uniqueIds.size !== ids.length) {
        ...
        throw new Error(`ID's must be unique, found duplicates for: ${duplicateIds}`);
    }
    return {
        ids,
        metadatas,
...
function wrapCollection(api, collection) {
    return new Collection(collection.name, collection.id, api, collection.embeddingFunction, collection.metadata);
}
// src/auth.ts
...
var authOptionsToAuthProvider = (auth)=>{
    ...
    switch(auth.provider){
        ...
        case "token":
            return new TokenAuthClientProvider(auth.credentials, auth.tokenHeaderType);
...
function isOfflineError(error) {
    var _a, _b, _c;
    return Boolean(((error == null ? void 0 : error.name) === "TypeError" || (error == null ? void 0 : error.name) === "FetchError") && (((_a = error.message) == null ? void 0 : _a.includes("fetch failed")) || ((_b = error.message) == null ? void 0 : _b.includes("Failed to fetch")) || ((_c = error.message) == null ? void 0 : _c.includes("ENOTFOUND"))));
}
function parseServerError(error) {
    ...
    return new ChromaServerError("The server encountered an error while handling the request.");
}
var chromaFetch = async (input, init)=>{
    try {
        ...
        if (!clonedResp.ok) {
            ...
            switch(resp.status){
                case 400:
                    throw new ChromaClientError(`Bad request to ${input} with status: ${resp.statusText}`);
                case 401:
                    throw new ChromaUnauthorizedError(`Unauthorized`);
                case 403:
                    throw new ChromaForbiddenError(`You do not have permission to access the requested resource.`);
                case 404:
                    throw new ChromaNotFoundError(`The requested resource could not be found: ${input}`);
                case 409:
                ...
                case 503:
                case 504:
                    throw new ChromaConnectionError(`Unable to connect to the chromadb server. Please try again later.`);
            }
            throw new Error(`Failed to fetch ${input} with status ${resp.status}: ${resp.statusText}`);
        }
...
var AdminClient = class {
   ...
   * });
   * ```
   */ constructor({ path, fetchOptions, auth, tenant = DEFAULT_TENANT, database = DEFAULT_DATABASE } = {}){
        this.tenant = DEFAULT_TENANT;
        this.database = DEFAULT_DATABASE;
        if (path === void 0) path = "http://localhost:8000";
        this.tenant = tenant;
   ...
   * });
   * ```
   */ async setTenant({ tenant = DEFAULT_TENANT, database = DEFAULT_DATABASE }) {
   ...
   * });
   * ```
   */ async setDatabase({ database = DEFAULT_DATABASE }) {
   ...
   */ async getTenant({ name }) {
        const getTenant = await this.api.getTenant(name, this.api.options);
        return {
            name: getTenant.name
        };
    }
   ...
   * });
   * ```
   */ async createDatabase({ name, tenantName }) {
        await this.api.createDatabase(tenantName, {
            name
        }, this.api.options);
        return {
            name
   ...
   * });
   * ```
   */ async getDatabase({ name, tenantName }) {
        const result = await this.api.getDatabase(name, tenantName, this.api.options);
        return result;
    }
    /**
   * Deletes a database.
   ...
   */ async deleteDatabase({ name, tenantName }) {
   ...
   */ async listDatabases({ limit, offset, tenantName }) {
        return await this.api.listDatabases(tenantName, limit, offset, this.api.options);
    }
};
    ...
    async generate(texts) {
        ...
        this.pipelinePromise = new Promise(async (resolve, reject)=>{
            try {
                ...
                resolve(await pipeline("feature-extraction", this.model, {
                    quantized,
                    revision,
                    progress_callback
                }));
    ...
    async loadClient() {
        ...
        try {
            ...
            if (_a.code === "MODULE_NOT_FOUND") {
                throw new Error("Please install the chromadb-default-embed package to use the DefaultEmbeddingFunction, `npm install chromadb-default-embed`");
            }
            throw _a;
        }
        this.transformersApi = TransformersApi;
    }
    /** @ignore */ static async import() {
        try {
            let importResult;
            ...
            ;
                ...
                importResult = await (()=>{
                    const e = new Error("Cannot find module 'chromadb-default-embed'");
            ...
            throw new Error("Please install chromadb-default-embed as a dependency with, e.g. `npm install chromadb-default-embed`");
        }
    }
};
// src/ChromaClient.ts
...
var ChromaClient = class {
   ...
   * });
   * ```
   */ constructor({ path = "http://localhost:8000", fetchOptions, auth, tenant = DEFAULT_TENANT2, database = DEFAULT_DATABASE2 } = {}){
        this.tenant = tenant;
        this.database = database;
        this.authProvider = void 0;
        const apiConfig = new Configuration({
    ...
    /** @ignore */ async init() {
        if (!this._initPromise) {
            ...
            this._initPromise = validateTenantDatabase(this._adminClient, this.tenant, this.database);
        }
   ...
   */ async getUserIdentity() {
        const user_identity = await this.api.getUserIdentity(this.api.options);
   ...
   * });
   * ```
   */ async createCollection({ name, metadata, embeddingFunction = new DefaultEmbeddingFunction() }) {
        await this.init();
   ...
   * });
   * ```
   */ async getOrCreateCollection({ name, metadata, embeddingFunction = new DefaultEmbeddingFunction() }) {
        await this.init();
        const newCollection = await this.api.createCollection(this.tenant, this.database, {
            name,
            // @ts-ignore: we need to generate the client libraries again
   ...
   */ async listCollections({ limit, offset } = {}) {
        await this.init();
        const collections = await this.api.listCollections(this.tenant, this.database, limit, offset, this.api.options);
   ...
   *    offset: 0,
   * });
   */ async listCollectionsAndMetadata({ limit, offset } = {}) {
        await this.init();
        return await this.api.listCollections(this.tenant, this.database, limit, offset, this.api.options);
    }
    /**
   ...
   */ async countCollections() {
        await this.init();
        return await this.api.countCollections(this.tenant, this.database, this.api.options);
    }
    /**
   ...
   */ async getCollection({ name, embeddingFunction }) {
        await this.init();
        const response = await this.api.getCollection(this.tenant, this.database, name, this.api.options);
        return wrapCollection(this, {
            name: response.name,
            id: response.id,
   ...
   */ async deleteCollection({ name }) {
        await this.init();
        await this.api.deleteCollection(name, this.tenant, this.database, this.api.options);
    }
};
// src/CloudClient.ts
var CloudClient = class extends ChromaClient {
    constructor({ apiKey, database, tenant, cloudHost, cloudPort }){
        if (!apiKey) {
...
var OpenAIEmbeddingFunction = class _OpenAIEmbeddingFunction {
    ...
    async loadClient() {
        ...
        try {
            ...
            if (_a.code === "MODULE_NOT_FOUND") {
                throw new Error("Please install the openai package to use the OpenAIEmbeddingFunction, e.g. `npm install openai`");
            }
            throw _a;
        }
    ...
    /** @ignore */ static async import() {
        try {
            const { default: openai } = await __turbopack_context__.A("[project]/node_modules/openai/index.mjs [app-route] (ecmascript, async loader)");
            const { VERSION } = await __turbopack_context__.A("[project]/node_modules/openai/version.mjs [app-route] (ecmascript, async loader)");
            return {
                openai,
                version: VERSION
            };
...
var CohereAISDK56 = class {
    ...
    async loadClient() {
        ...
        const { default: cohere } = await __turbopack_context__.A("[project]/node_modules/cohere-ai/index.js [app-route] (ecmascript, async loader)");
...
var CohereAISDK7 = class {
    ...
    async loadClient() {
        if (this.cohereClient) return;
        const cohere = await __turbopack_context__.A("[project]/node_modules/cohere-ai/index.js [app-route] (ecmascript, async loader)").then((cohere2)=>{
            return cohere2;
        });
...
var CohereEmbeddingFunction = class {
    ...
    async initCohereClient() {
        ...
        try {
            this.cohereAiApi = await __turbopack_context__.A("[project]/node_modules/cohere-ai/index.js [app-route] (ecmascript, async loader)").then((cohere)=>{
                if (cohere.CohereClient) {
                    return new CohereAISDK7({
                        apiKey: this.apiKey
                    });
                } else {
                    return new CohereAISDK56({
                        apiKey: this.apiKey
                    });
                }
            });
   ...
   */ constructor({ model = "Xenova/all-MiniLM-L6-v2", revision = "main", quantized = false, progress_callback = null } = {}){
    ...
    async loadClient() {
        ...
        try {
            ...
            if (_a.code === "MODULE_NOT_FOUND") {
                throw new Error("Please install the @xenova/transformers package to use the TransformersEmbeddingFunction, `npm install @xenova/transformers`");
            }
            throw _a;
        }
    ...
    /** @ignore */ static async import() {
        try {
            const { pipeline } = await (()=>{
                const e = new Error("Cannot find module '@xenova/transformers'");
                e.code = 'MODULE_NOT_FOUND';
                throw e;
            })();
            return {
            ...
            throw new Error("Please install @xenova/transformers as a dependency with, e.g. `npm install @xenova/transformers`");
        }
    }
};
...
var GoogleGenerativeAiEmbeddingFunction = class _GoogleGenerativeAiEmbeddingFunction {
    ...
    async loadClient() {
        ...
        try {
            ...
            if (_a.code === "MODULE_NOT_FOUND") {
                throw new Error("Please install the @google/generative-ai package to use the GoogleGenerativeAiEmbeddingFunction, `npm install @google/generative-ai`");
            }
            throw _a;
        }
    ...
    async generate(texts) {
        await this.loadClient();
        const model = this.googleGenAiApi.getGenerativeModel({
            model: this.model
        });
    ...
    /** @ignore */ static async import() {
        try {
            ...
            throw new Error("Please install @google/generative-ai as a dependency with, e.g. `npm install @google/generative-ai`");
        }
    }
};
...
var OllamaEmbeddingFunction = class _OllamaEmbeddingFunction {
    constructor({ url = DEFAULT_LOCAL_URL, model = DEFAULT_MODEL } = {
    ...
    async initClient() {
        ...
        try {
            ...
            this.ollamaClient = new ollama.Ollama({
                host: this.url
            });
        } catch (e) {
            if (e.code === "MODULE_NOT_FOUND") {
                throw new Error("Please install the ollama package to use the OllamaEmbeddingFunction, `npm install -S ollama`");
            }
            throw e;
        }
    }
    /** @ignore */ static async import() {
        try {
            const { ollama } = await (()=>{
                const e = new Error("Cannot find module 'ollama'");
                e.code = 'MODULE_NOT_FOUND';
                throw e;
            })().then((m)=>({
                    ollama: m
            ...
            throw new Error("Please install Ollama as a dependency with, e.g. `npm install ollama`");
        }
    }
    async generate(texts) {
...
//# sourceMappingURL=node_modules_chromadb_dist_chromadb_mjs_8a6fdb40._.js.map